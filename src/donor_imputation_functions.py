...
def nearest_neighbour_imputation_categorical(
    ...
    numeric_imputation_class_vars = list(set(imputation_class_vars).difference(categorical_imputation_class_vars))
    ...
    data_post_imputation_by_slices = {
        ...
        for slice, sub_df in data_post_imputation.groupby(categorical_imputation_class_vars)
    }

    # get column numbers for indexing use later
    data_columns = data_post_imputation_by_slices[[k for k in data_post_imputation_by_slices.keys()][0]].columns
    ...
    for slice, subset_data in data_post_imputation_by_slices.items():
        ...
        for tree_index_num, data_index_num in enumerate(subset_data.index):
            ...
            if (pd.notna(subset_data[post_imputation_col_name][data_index_num]) and 
                (subset_data[post_imputation_col_name][data_index_num] in missing_values_to_impute)):
                ...
                while continue_neighbour_search:
                    ...
                    if tuple(subset_data.iloc[data_index_num][categorical_imputation_class_vars]) == slice:
                        neighbour_distances_indices = neighbour_distances_indices_by_slices[slice]
                        ...
                        elif data_post_imputation_by_slices[slice][post_imputation_col_name][donor_idx] in missing_values_to_impute:
                            neighbour_rank += 1
                        ...
                        # else, donor is eligible; end neighbour search
                            ...
                            data_post_imputation_by_slices[slice].iloc[
                                ...
                                ] = data_post_imputation_by_slices[slice][post_imputation_col_name][donor_idx]
                        
                            # indicate imputation
                            data_post_imputation_by_slices[slice].iloc[data_index_num, imputation_flag_col_idx] = 1
        
                            # indicate type of imputation
                            data_post_imputation_by_slices[slice].iloc[data_index_num, imputation_type_col_idx] = 'KNN'
                            ...
                            data_post_imputation_by_slices[slice].iloc[data_index_num, donor_userid_col_idx] = donor_userid
                            ...
                            for n, var in enumerate(imputation_class_vars):
                                ...
                                ] = data_post_imputation_by_slices[slice][var][donor_idx]
        
                            # save donor distance
                            data_post_imputation_by_slices[slice].iloc[data_index_num, donor_distance_col_idx] = donor_dist
    ...
    for row_num, user_id in enumerate(data_post_imputation[donor_userid_col_name]):
        if user_id != -1:
            data_post_imputation.iloc[row_num, (donor_count_col_idx - 1)] = donor_use_track_dict[user_id]  
...
def nearest_neighbour_imputation(
    ...
    data_post_imputation[imputation_class_vars_w_na] = data_post_imputation[imputation_class_vars].replace(missing_values_to_impute, pd.NA)
    data_post_imputation[post_imputation_col_name_w_na] = data_post_imputation[post_imputation_col_name].replace(missing_values_to_impute, pd.NA)
    
    if multiple_possible_class_vars is not None:
        multiple_possible_class_vars_w_na = [f'{var}_w_na' for var in multiple_possible_class_vars]
        data_post_imputation[multiple_possible_class_vars_w_na] = data_post_imputation[multiple_possible_class_vars].replace(missing_values_to_impute, pd.NA)
    ...
    categorical_imputation_class_vars = list(
        knn_impute_subset[imputation_class_vars].columns[knn_impute_subset[imputation_class_vars].dtypes == 'object']
    )
    if categorical_imputation_class_vars:
        ...
        imputation_class_vars_transformed = minmax_scaler.fit_transform(knn_impute_subset[imputation_class_vars_w_na])
        ...
        for tree_index_num, data_index_num in enumerate(knn_impute_subset.index):
            ...
            if (pd.notna(knn_impute_subset[post_imputation_col_name][data_index_num]) and 
                (knn_impute_subset[post_imputation_col_name][data_index_num] in missing_values_to_impute)):
                ...
                while continue_neighbour_search:
                    ...
                    # third check: if donor has missing value, then continue neighbour search
                    elif knn_impute_subset[post_imputation_col_name][donor_idx] in missing_values_to_impute:
                        neighbour_rank += 1
                    ...
                    # else, donor is eligible; end neighbour search
                        ...
                        # impute value into post-imputation variable
                        knn_impute_subset.iloc[data_index_num, post_imputation_col_idx] = knn_impute_subset[post_imputation_col_name][donor_idx]
                    
                        # indicate imputation
                        knn_impute_subset.iloc[data_index_num, imputation_flag_col_idx] = 1
    
                        # indicate type of imputation
                        knn_impute_subset.iloc[data_index_num, imputation_type_col_idx] = 'KNN'
    
                        # save imputed value
                        knn_impute_subset.iloc[data_index_num, imputed_val_col_idx] = knn_impute_subset[post_imputation_col_name][donor_idx]
    
                        # save donor's user id
                        knn_impute_subset.iloc[data_index_num, donor_userid_col_idx] = donor_userid
                        ...
                        # save donor characteristics
                        for n, var in enumerate(imputation_class_vars):
                            knn_impute_subset.iloc[data_index_num, donor_characteristics_col_idxs[n]] = knn_impute_subset[var][donor_idx]
    
                        # save donor distance
                        knn_impute_subset.iloc[data_index_num, donor_distance_col_idx] = donor_dist

                        # populate donor use count column
                        for row_num, user_id in enumerate(knn_impute_subset[donor_userid_col_name]):
                            if user_id != -1:
                                knn_impute_subset.iloc[row_num, donor_count_col_idx] = donor_use_track_dict[user_id]  
                        ...
                        if bucket_imputation:
                            ...
                            knn_impute_subset[bucketed_post_imputation_col_name] = continuous_to_range(
                                knn_impute_subset,
                        ...
                        knn_impute_subset = knn_impute_subset.set_index('original_index')
    ...
    # then separate distance matrices need to be computed per variable in multiple_possible_class_vars
        ...
        for var in multiple_possible_class_vars_w_na:
            ...
            # subset data to remove blanks and potential donors with missing imputation class variables
            knn_impute_subset = data_post_imputation[
                    (data_post_imputation[imputation_class_vars_w_na + var].notna().all(axis=1))
            ]

            # Need to min-max scale imputation class variables before determining nearest neighbours.
            imputation_class_vars_transformed = minmax_scaler.fit_transform(knn_impute_subset[imputation_class_vars_w_na + var])
                ...
                while continue_neighbour_search:
                    ...
                    while continue_class_vars_search:
                        ...
                        if tree_to_use >= len(multiple_possible_class_vars):
                            ...
                            userid_missing_class_vars = knn_impute_subset[data_index_num][userid_col_name]
                            warnings.warn(
                                f'All variables in multiple_possible_class_vars are missing for user id {userid_missing_class_vars}.'
                            )
                        
                        elif pd.isna(knn_impute_subset[multiple_possible_class_vars[tree_to_use]]):
                            tree_to_use += 1
                    ...
                    neighbour_distances_indices = list_neighbour_distances_indices[tree_to_use]
