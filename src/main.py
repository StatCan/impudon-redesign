...
from donor_imputation_functions import data_preprocessing_store_donor_info, continuous_to_range, range_to_random_continuous, monetary_buckets, nearest_neighbour_imputation_categorical, nearest_neighbour_imputation
from historical_imputation_functions import  ratio_imputation, construct_house_loan_donor_pool_one_house, construct_house_loan_donor_pool_multiple_houses, construct_incomes_donor_pool, housing_historical_imputation_by_row, historical_imputation_housing,  income_historical_imputation_by_row,  historical_imputation_income
...
def main(config = config): 
    ...
    if config['IMPUTATION']['DONOR_IMPUTATION']:
        ...
        with tqdm(total = len(var_to_impute_list), desc = 'Processing donor imputation') as progress_bar:
            with pd.ExcelWriter(config['PATHS']['output_dir'] / config['PATHS']['output_file_donor']) as writer:
    
                df_data_donor.to_excel(writer, sheet_name = 'Original Data', index=False)
    
                for var_to_impute in var_to_impute_list:

                    imputation_class_vars = config['IMPUTATION']['DONOR_IMPUTATION'][var_to_impute]
                    df_data_donor_preprocessed = data_preprocessing_store_donor_info(df_data_donor, var_to_impute, imputation_class_vars)
                    ...
                    df_data_donor_post_knn.to_excel(writer, sheet_name = var_to_impute, index=False)
    ...
    if config['IMPUTATION']['HISTORICAL_IMPUTATION']['HOUSING_VARS']:
        ...
        var_to_impute_list = list(config['IMPUTATION']['HISTORICAL_IMPUTATION']['HOUSING_VARS'].keys())

        print(f"\n\nHousing variables to be imputed with historical imputation : {', '.join(var_to_impute_list)}")
        with tqdm(total = len(var_to_impute_list), desc = 'Processing housing imputation') as progress_bar:

            with pd.ExcelWriter(config['PATHS']['output_dir'] / config['PATHS']['output_file_hist_housing']) as writer:
    
                df_data_hist.to_excel(writer, sheet_name = 'Original Data', index=False)

                for var_current_wave in var_to_impute_list:
                    var_previous_wave = config['IMPUTATION']['HISTORICAL_IMPUTATION']['HOUSING_VARS'][var_current_wave]['var_housing_loan_previous_wave']
                    var_number_properties_current_wave = config['IMPUTATION']['HISTORICAL_IMPUTATION']['HOUSING_VARS'][var_current_wave]['var_number_properties_current_wave']
                    var_number_properties_previous_wave = config['IMPUTATION']['HISTORICAL_IMPUTATION']['HOUSING_VARS'][var_current_wave]['var_number_properties_previous_wave']
                    var_hdb_indicator_current_wave = config['IMPUTATION']['HISTORICAL_IMPUTATION']['HOUSING_VARS'][var_current_wave]['var_hdb_indicator_current_wave']
                    var_hdb_indicator_previous_wave = config['IMPUTATION']['HISTORICAL_IMPUTATION']['HOUSING_VARS'][var_current_wave]['var_hdb_indicator_previous_wave']
                    imputation_class_vars = config['IMPUTATION']['HISTORICAL_IMPUTATION']['HOUSING_VARS'][var_current_wave]['imputation_class_vars']
                    values_to_impute = config['IMPUTATION']['HISTORICAL_IMPUTATION']['HOUSING_VARS'][var_current_wave]['values_to_impute']
                    ...
                    df_data_hist_post_imputation[df_data_hist_post_imputation[f'{var_current_wave}_imputation_flag'] == 1][
                        [
                        ...
                        ].to_excel(writer, sheet_name = var_current_wave, index=False)
                    progress_bar.update(1)
    ...
    if config['IMPUTATION']['HISTORICAL_IMPUTATION']['INCOME_VARS']:
        ...
        with tqdm(total = len(var_to_impute_list), desc = 'Processing income imputation') as progress_bar:

            with pd.ExcelWriter(config['PATHS']['output_dir'] / config['PATHS']['output_file_hist_income']) as writer:
